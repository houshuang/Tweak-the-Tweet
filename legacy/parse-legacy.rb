...
# changes to records on the spreadsheet are updated into the MySQL record
# storage this can cause things to be way out of line, if someone is not
# updating the Spreadsheet correctly - so it's currently dangerous I would only
# allow this portion if I was closely monitoring the event (and giving only
# select access to edit the spreadsheet)
...
# Comparse two strings for similarity and return a string that unifies both.
# That is, return one string if it contains the other, or return string
# concatenation of the both.
def collapse(s1, s2)
  # return the non-blank string if one is blank
  return s2 if s1 == nil
  return s1 if s2 == nil

  # strip out irrelevant text (e.g. (cont), "...", "NA"
  x = s2.index("(cont)")
  if x != nil && x > 0
    s2 = s2[0...x]
  end
  ...
  # if one string is contained within the other, return the larger string
  if s1.index(s2) != nil
    return s1
  end
  ...
  # otherwise, return both strings concatenated together
  return s1 + " / " + s2
end
...
def parse_tweets_to_database(db, time)
  # remove me? del = db.query("DELETE FROM `tweets` WHERE `text` LIKE '%[where:%' OR `text` LIKE '%[type%'") 
  ...
  dis.each do |group|
    ...
    # begin processing the tweet text
    ...
    # if there is a place_url (from tweet metadata) - then take that instead of
    # the google_loc   
         ...
         aRes.each do |row|
          ...
          if count == 0  
            db.query("INSERT INTO records (type, type_specifics, text_location, contact, source, num, status, pic, info, time, tweet_author, tweet, gps_lat, gps_long, loc_url, location, event) VALUES ('" + type + "', '" +
                ...
                pic + "', '" + info + "', '" + time + "', '" + author + "', '" + twt + "', '" + gps_lat +
